...
class ECOrderController extends Controller
{
    ...
    {
        return view('user.ecommerce.order.index',compact('status'));
    }
    ...
    public function addPayment(string $id)
    {
        ...
        return view('user.ecommerce.order.add_payment', compact('order', 'paymentMethods', 'recieverAddresses'));
    }
    ...
    public function edit(string $id)
    {
        ...
        $order = EcommerceOrder::findOrFail($id);
        return view('user.ecommerce.order.edit', compact('order','paymentStatuses','configStatuses'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        //     } else {
        //         $notify = [
        //             'error' => "Something went wrong contact your user.",
        //         ];
        //         return $notify;
        //     }
        // }
    ...
    public function getPaymentHtml($id)
    {
        ...
        $paymentStatuses = PaymentStatus::get();
        return view('user.ecommerce.order.payment', ['payment' => $payment, 'paymentStatuses' => $paymentStatuses]);
    }
    ...
    public function show(string $id)
    {
        ...
        return view('user.ecommerce.order.show', ['order' => $order, 'carts' => $carts, 'cal' => $cal]);
    }
    ...
    public function printInvoice($id)
    {
        ...
        return view('user.ecommerce.order.print_invoice', compact('order', 'carts', 'barcode', 'cal'));
    }
    ...
    public function data(Request $request)
    {
        if($request->status == 'pending')
            $data = EcommerceOrder::where('user_id',Auth::user()->id)->where('status','new')->orWhere('status','process')->get();
        elseif($request->status == 'delivered')
            $data = EcommerceOrder::where('user_id',Auth::user()->id)->where('status','delivered')->get();
        elseif($request->status == 'cancelled')
            $data = EcommerceOrder::where('user_id',Auth::user()->id)->where('status','cancel')->get();
        else
            $data = EcommerceOrder::where('user_id',Auth::user()->id)->get();
            ...
            ->addColumn('action', function ($row) {
                $actionBtn = '<div class="dropdown dropstart">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="margin: 0px;">
                        <li><a class="dropdown-item order-data-view" href="' . route('user.ec-order.show', ['id' => $row->id]) . '">View</a></li> 
                        <li><a class="dropdown-item order-data-invoice" href="' . route('user.ec-order.invoice.print', ['id' => $row->id]) . '" target="_blank">Print Invoice</a></li>
                    </ul>
                </div>';
                return $actionBtn;
            })
