...
class ParcelController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        $data = Parcel::where('user_id', Auth::user()->id)->where('parcel_status_id','!=',5)->whereNotIn('id', $ids);
        ...
        return Datatables::of($data)
            ...
            ->addColumn('invoice_status', function ($row) {
                ...
                if ($row->show_invoice == 0) {
                    $html = 'Missing Invoice';
                } else {
                    $html = '<a  href="' . route('user.parcel.invoice', ['id' => $row->id]) . '" target="_blank"><span class="mdi mdi-file-document-box"></span></a>';
                }
