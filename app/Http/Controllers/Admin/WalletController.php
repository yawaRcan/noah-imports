...
use App\Http\Requests\Admin\AdminWallet;
...
class WalletController extends Controller
{
    public function index()
    {
        ...
        $config = '';
        return view('admin.wallet.index', ['status' => $config,'users'=>$users,'currencies'=>$currencies,'paymentMethods'=> $paymentMethods]);
    }
    ...
    public function getPendingHtml($id)
    {
       
         $wallet = Wallet::findOrFail($id);
         return view('admin.wallet.pending-reject', ['wallet' => $wallet]);
    }
    ...
    public function data(Request $request)
    {
        ...
        return Datatables::of($data)
            ...
            ->addColumn('status', function ($row) {
                ...
                if ($row->status == 'pending')
                    $html = '<span class="mb-1 badge bg-warning ni-pending-show-modal" data-wallet-id="' . $row->id . '">Pending</span>';
