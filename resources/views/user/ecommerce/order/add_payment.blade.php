                ...
                <div class="border-bottom title-part-padding">
                    <div class="row">
                        ...
                        <div class="col-6 text-end">
                            <p class="card-title mb-0">Total Amount to be paid: <strong>ƒ {{$order->total_converted}} ANG</strong></p>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <select class="form-control reciever_address select2" name="shipping_address_id" style="width: 90%;">
                                    ...
                                    <option value="{{$recieverAddress->id}}" {{$order->shipping_id == $recieverAddress->id ? 'selected' : ''}}>{{$recieverAddress->first_name}} {{$recieverAddress->last_name}} - {{$recieverAddress->country->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    $(document).on('click', "#payment-update-btn", function() {
        ...
        let url = "{{route('user.ec-order.payment.update', ['id' => ':id'])}}";
    ...
    $(document).on('change', "#payment_method", function() {
        ...
        let url = "{{route('user.ec-order.get.paymentInfo',['id' => ':id','user_id' => ':user_id'])}}";
        url = url.replace(':id', id);
