...
<div class="modal fade" id="ni-pending-change-modal" tabindex="-1" aria-labelledby="ni-pending-change-modal" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header d-flex align-items-center">
                <h4 class="modal-title" id="myLargeModalLabel">Accept/Reject Wallet</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" id="pending-change-body">
...
<script>
    ...
    $(document).on('click', ".ni-pending-show-modal", function() {
        
        let id = $(this).data("wallet-id");
        let url = "{{route('wallet.getPendingHtml',['id' => ':id'])}}";
        url = url.replace(':id', id);
        // Function to get html of Payment Data
        getHtmlAjax(url, "#ni-pending-change-modal", "#pending-change-body")
    })
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.value) {
                ...
                request.done(function(response) {
                    if (response.success) {
                        $('#ni-pending-change-modal').modal('hide');
