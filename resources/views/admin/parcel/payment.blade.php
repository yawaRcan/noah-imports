                ...
                <div class="input-group mb-3">
                    ...
                    <select name="payment_status"  class="form-control" id="payment_status" >
                        @foreach($paymentStatuses as $status)
                            @if($status->slug == 'paid' || $status->slug == 'reject')
                                <option value="{{$status->id}}" {{ $payment->payment_status_id == $status->id ? 'selected': '' }}>{{$status->name}}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
                <div class="col-12 reject_reason" style="display:none;" >
                <label>Reason </label> 
                <div class="input-group mb-3">
               
                    <textarea class="form-control reject_reason"  id="reject_reason" style="width:100%;display:none;" name="reason" rows="4"></textarea>
                </div> 
                </div>
...
<script> 
    ...
    $("#payment_status").on('change', function(e) {
        if($(this).val()==5){
             $(".reject_reason").attr('style','display:block !important');
        }else{
            $(".reject_reason").attr('style','display:none !important');
