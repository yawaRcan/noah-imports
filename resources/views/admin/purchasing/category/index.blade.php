...
<div class="row page-titles">
    <div class="col-md-5 col-12 align-self-center">
        <h3 class="text-themecolor mb-0">Purchase</h3>
                ...
                <div class="border-bottom title-part-padding">
                    <div class="row">
                        ...
                        <div class="col-6 text-end">
                            <button type="button" class="btn btn-light-info text-info font-weight-medium waves-effect category-data-add">Add Category</button>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function table(params = null) {
        ...
        table = $('.data-table').DataTable({
            ...
            destroy: true,
            stateSave: true,
            ajax: "{{route('purchase.category.data')}}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
    ...
    $(document).on('click', ".category-data-add", function() {
        let url = "{{route('purchase.category.create')}}";
    ...
    $(document).on('click', "#category-add-button", function() {
        ...
        let url = "{{route('purchase.category.store')}}";
    ...
    $(document).on('click', ".category-data-delete", function() {
        ...
        let url = "{{route('purchase.category.destroy',['id' => ':id'])}}";
    ...
    $(document).on('click', ".category-data-edit", function() {
        ...
        let id = $(this).data("category-id");
        let url = "{{route('purchase.category.edit',['id' => ':id'])}}";
        url = url.replace(':id', id);
    ...
    $(document).on('click', "#category-edit-button", function() { 
        let id = $(this).data("category-id");
        let url = "{{route('purchase.category.update',['id' => ':id'])}}";
        url = url.replace(':id', id);
