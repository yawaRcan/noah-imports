                ...
                <div class="card-body" style="min-height:273px">
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-6 mb-2">
                            <div class="btn btn-primary btn-sm btn-rounded w-100"> Username :<br> {{$user->username ?? ''}}</div>
                        </div>
                        <div class="col-12 col-sm-12 col-md-6  mb-2">
                            <div class="btn btn-success btn-sm btn-rounded w-100"> Gender : <br> {{$user->gender == 1 ? 'Male':'Female' ?? 'Other'}}</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-6  mb-2">
                            <div class="btn btn-success btn-sm btn-rounded w-100"> Mobile number : <br> {{$user->phone ?? '+923345678756'}}</div>
                        </div>
                        <div class="col-12 col-sm-12 col-md-6  mb-2">
                            <div class="btn btn-primary btn-sm  btn-rounded w-100"> Country :<br> {{$user->country->name ?? 'United State America'}}</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-6  mb-2">
                            <div class="btn btn-primary btn-sm  btn-rounded w-100"> Registered At :<br> {{ \Carbon\Carbon::parse($user->created_at)->setTimezone((isset($user->timezone->name) ? $user->timezone->name : 'US/Samoa'))->format('Y-m-d') ?? ''}}</div>
                        </div>
                        <div class="col-12 col-sm-12 col-md-6  mb-2">
                            <div class="btn btn-success btn-sm  btn-rounded w-100"> Last Login : <br> {{\Carbon\Carbon::parse($user->last_login)->setTimezone((isset($user->timezone->name) ? $user->timezone->name : 'US/Samoa'))->diffForHumans() ?? ''}}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body shadow">
                    ...
                    <!-- Tab panes -->
                    <div class="tab-content border mt-2 table-responsive">
...
<script>
    ...
    function walletTable(params = null) {
        ...
        walletTable = $('#wallet-data-table').DataTable({
            ...
            destroy: true,
            stateSave: true,
            ajax: "{{route('wallet.transaction.data')}}?user_id=" + "{{$user->id}}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
